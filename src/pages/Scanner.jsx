  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        paddingRight: "20px", borderRight: "1px solid rgba(201,168,76,0.2)",
        marginRight: "16px", flexShrink: 0,
      }}>FUTURES</span>

      {FUTURES_LIST.map((f, i) => {
        const d = data[f.symbol];
        const price = d?.last ?? null;
        const chg = d?.change ?? null;
        const pct = d?.changePct ?? null;
        const isUp = chg === null ? null : chg >= 0;
        const color = isUp === null ? "rgba(255,255,255,0.45)" : isUp ? "#22c55e" : "#ef4444";

        return (
          <div key={f.symbol} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "0 16px" }}>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", fontWeight: "700", color: "#C9A84C" }}>{f.label}</span>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "13px", fontWeight: "600", color: "#fff" }}>
                {error ? "—" : price === null ? "···" : price.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
              {!error && chg !== null && (
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color }}>
                  {isUp ? "▲" : "▼"} {chg >= 0 ? "+" : ""}{chg.toFixed(2)} ({pct >= 0 ? "+" : ""}{pct.toFixed(2)}%)
                </span>
              )}
            </div>
            {i < FUTURES_LIST.length - 1 && <div style={{ width: "1px", height: "28px", background: "rgba(201,168,76,0.12)", flexShrink: 0 }} />}
          </div>
        );
      })}

      {lastUpdate && (
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", color: "rgba(255,255,255,0.3)", whiteSpace: "nowrap", marginLeft: "auto", paddingLeft: "16px" }}>
          {error ? "⚠ error" : `↻ ${lastUpdate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}`}
        </span>
      )}
    </div>
  );
